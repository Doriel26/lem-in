...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/01/02 15:50:13 by dchiche           #+#    #+#             */
/*   Updated: 2018/01/16 16:45:53 by dchiche          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int		ft_min_x_maj(t_flag new_struct, uintmax_t nb, int len)
{
	int diff;
	int diffwidth;
	int tmp;
...
int		ft_zero_x_maj(t_flag new_struct, uintmax_t nb, int len)
{
	int diff;
	int diffwidth;
	int tmp;
...
int		ft_classic_x_maj(t_flag n_s, uintmax_t nb, int len, int tmp)
{
	int diff;
	int diffwidth;
	int tmpwidth;
...
int		ft_x_maj(t_flag new_struct, va_list ap)
{
	...
	if (ft_strchr(new_struct.flag, '-'))
		res = ft_min_x_maj(new_struct, nb, len);
	else if (ft_strchr(new_struct.flag, '0') && new_struct.precision == -1)
		res = ft_zero_x_maj(new_struct, nb, len);
	else
		res = ft_classic_x_maj(new_struct, nb, len, tmp);
	return (res);
}
